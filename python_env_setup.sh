...
# Cannot source .bashrc in script, .bashrc exits w/o running
# Making tmp file to store additions, then adding to RC_FILE
TMP_FILE=~/temp_file_delete_if_seen.sh
touch $TMP_FILE
...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
  ...
  if [ -d ~/.pyenv ]; then
    echo ""
    echo "WARNING: pyenv already installed? Directory in ~/.pyenv, not re-installing."
    echo "Check ~/.bashrc for duplicate code"
    echo ""
  else
    curl https://pyenv.run | bash
  fi
  echo 'export PYENV_ROOT="$HOME/.pyenv"'>> $TMP_FILE
  echo 'export PATH="$HOME/.pyenv/bin:$PATH"' >> $TMP_FILE
elif [[ "$OSTYPE" == "darwin"* ]]; then
  brew update
...
if [ -f ~/.direnv ]; then
  echo ""
  echo "WARNING: file ~/.direnv already exists. Check that there are no duplications"
  echo ""
fi
cat <<EXCL >> ~/.direnvrc
use_python() {
...
if [ -f ~/.gitignore_global ]; then
  echo ""
  echo "WARNING: file ~/.gitignore_global already exists. Check that there are no duplications"
  echo ""
fi
